...
const COLLECTIONS = ["recipes", "ingredientes", "avisos", "proyectos", "eventos", "proveedores", "practicantes", "centros", "habitaciones", "pedidosHistorial", "descargables", "empresas", "grupo_descargables", "oba_recetas", "oba_menus", "oba_ideas", "oba_kpis", "ene_recetas", "ene_menus", "ene_ideas", "ene_kpis", "candomo_recetas", "candomo_menus", "candomo_ideas", "candomo_kpis", "canitas_recetas", "canitas_menus", "canitas_ideas", "canitas_kpis", "cebo_recetas", "cebo_menus", "cebo_ideas", "cebo_kpis", "huerta_plantas", "inventario", "staff_meal"];
...
function renderAll() {
  ...
  const fns = [
    rInicio, rRec, rPedLista, rStaffMeal, calRender, rPrac, rProj, rAv, rGrupo,
...
// ── Staff meal semanal (jueves a domingo) ───────────
const STAFF_MEAL_DIAS = [
...
function _staffMealMonday(weekOffset) {
  const d = new Date();
  d.setHours(0, 0, 0, 0);
...
function _staffMealDateForDia(weekOffset, offsetDays) {
  const d = new Date(_staffMealMonday(weekOffset));
  d.setDate(d.getDate() + offsetDays);
  return d;
}

function _staffMealISO(date) {
  const y = date.getFullYear(), m = String(date.getMonth() + 1).padStart(2, "0"), d = String(date.getDate()).padStart(2, "0");
  return `${y}-${m}-${d}`;
}

// La semana se identifica por la fecha (ISO) de su jueves.
function _staffMealWeekKey(weekOffset) {
  return _staffMealISO(_staffMealDateForDia(weekOffset, 3));
}

function _staffMealRangeLabel(weekOffset) {
  const jue = _staffMealDateForDia(weekOffset, 3);
  const dom = _staffMealDateForDia(weekOffset, 6);
  const fmt = (d) => `${d.getDate()} ${MESES[d.getMonth()].slice(0, 3).toLowerCase()}`;
  return `${fmt(jue)} – ${fmt(dom)}`;
}

function staffMealWeekNav(delta) {
  staffMealWeekOffset += delta;
  rStaffMeal();
}

function staffMealThisWeek() {
  staffMealWeekOffset = 0;
  rStaffMeal();
}

function staffMealSetDia(weekOffset, dia, valor) {
  const key = _staffMealWeekKey(weekOffset);
  if (!D.staff_meal) D.staff_meal = [];
  let row = D.staff_meal.find((r) => r.semana === key);
  if (!row) {
    row = { id: nid++, semana: key, jueves: "", viernes: "", sabado: "", domingo: "" };
    D.staff_meal.push(row);
  }
  row[dia] = valor;
  save("staff_meal");
}
...
function rStaffMeal() {
  const body = document.getElementById("staffmeal-body");
  if (!body) return;
  const key = _staffMealWeekKey(staffMealWeekOffset);
  const row = (D.staff_meal || []).find((r) => r.semana === key) || {};
  const todayISO = _staffMealISO(new Date());

  const days = STAFF_MEAL_DIAS.map((d) => {
    const date = _staffMealDateForDia(staffMealWeekOffset, d.offset);
    const iso = _staffMealISO(date);
    const isToday = iso === todayISO;
    const dateLabel = `${date.getDate()} ${MESES[date.getMonth()].slice(0, 3).toLowerCase()}`;
    const val = row[d.key] || "";
    return `
      <div class="staffmeal-day${isToday ? " is-today" : ""}">
        <div class="staffmeal-day-head">
          <span class="staffmeal-day-name">${d.label}</span>
          <span class="staffmeal-day-date">${dateLabel}</span>
        </div>
        <input class="staffmeal-input" value="${safeText(val)}" placeholder="Sin definir"
               onchange="staffMealSetDia(${staffMealWeekOffset},'${d.key}',this.value)">
      </div>`;
  }).join("");

  body.innerHTML = `
    <div class="staffmeal-card">
      <div class="staffmeal-head">
        <div>
          <div class="eyebrow">Staff meal</div>
          <h3>Menú de la semana</h3>
        </div>
        <div class="staffmeal-nav">
          <button class="staffmeal-nav-btn" onclick="staffMealWeekNav(-1)" aria-label="Semana anterior">‹</button>
          <span class="staffmeal-range">${_staffMealRangeLabel(staffMealWeekOffset)}</span>
          <button class="staffmeal-nav-btn" onclick="staffMealWeekNav(1)" aria-label="Semana siguiente">›</button>
        </div>
      </div>
      <div class="staffmeal-days">${days}</div>
      ${staffMealWeekOffset !== 0 ? `<button class="ghost-btn ghost-btn-sm staffmeal-today-btn" onclick="staffMealThisWeek()">Ir a esta semana</button>` : ""}
    </div>`;
}
